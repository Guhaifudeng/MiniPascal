...
bool semantic007()
{
	...
	if (!iTypePos.empty()
		&& iTypePos.top()!=-1
		&& SymbolTbl.TypeInfoTbl[iTypePos.top()].m_eDataType==StoreType::T_NONE)	//标量类型
	{
...
bool semantic008()
{
	...
	if (i!=-1)
	{
		if (SymbolTbl.TypeInfoTbl[i].m_iState!=1)
		{
	...
	if (TmpResult!=-1)
	{
		...
		else
		{
			...
			TmpRslt.m_iLink=SymbolTbl.GetTmpVar(SymbolTbl.ProcStack.top()
								,(StoreType::CStoreType)SymbolTbl.TypeSysTbl.at(TmpResult).m_iRsltType);
		...
		&& CurrentVar.top().m_eOffsetType!=OffsetType::NoneOffset)
	{
		...
		if (!SymbolTbl.IsTmpVar(Tmp.m_iLink))
		{
				...
				&& CurrentVar.top().m_bRef==false)
			{
				...
				SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(GETADDR,Tmp,TmpRslt));
				Tmp=TmpRslt;
			}
			else
			{
		...
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(ADD_4,Op1,Tmp,Rslt));
		if (iConstOffset!=0)
		{
			Op1.m_iType=OpInfo::CONST;
			Op1.m_iLink=SymbolTbl.RecConstTbl(itos(iConstOffset),3);
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(ADD_4,Rslt,Op1,Rslt));
		}
...
bool semantic054()
{
	...
	for (;j>=0;j--)
	{
		...
		for(i=0;i<SymbolTbl.TypeInfoTbl[getWithStackElement(WithStack,j).m_Var.m_VarTypeStack.top().m_iLink].m_FieldInfo.size();i++)
		{
			if (SymbolTbl.TypeInfoTbl[getWithStackElement(WithStack,j).m_Var.m_VarTypeStack.top().m_iLink]
		...
		if (i!=SymbolTbl.TypeInfoTbl[getWithStackElement(WithStack,j).m_Var.m_VarTypeStack.top().m_iLink].m_FieldInfo.size())
		{
			break;
		}
	}
	...
	if (j>=0)
	{
		CurrentVar.push(getWithStackElement(WithStack,j).m_Var);
...
bool semantic056()
{
	...
	if (i!=SymbolTbl.TypeInfoTbl[TmpVar.m_VarTypeStack.top().m_iLink].m_FieldInfo.size())
	{
		...
		OffsetType::COffsetType TmpOffsetType=OffsetType::ConstOffset;
...
bool semantic057()
{
	if (iExpListFlag.top()==1)
	{
		...
		else
		{
			...
			if (!GenIR(Tmp1,Tmp2,TmpRslt,ADD))
			{
				return false;
			}
...
bool semantic058()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(ASSIGN_4,TmpOp1,TmpRslt));
	TmpOp.m_bRef=false;
...
bool semantic059()
{
	...
	else
	{
		...
		if (!GenIR(TmpOp1,TmpOp2,TmpRslt,SUB))
		{
			return false;
		}
		...
		if (!GenIR(TmpOp1,TmpOp2,TmpRslt,MUL))
		{
			return false;
		}
...
bool semantic060()
{
	...
	if (Tmp.m_iType==OpInfo::PROC)
	{
		...
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(GETPROCADDR,Op1,Rslt));
		Operand.push(Rslt);
		return true;
	}
	...
	if (!Tmp.m_bRef)
	{
		...
		{
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(ASSIGN_4,Op1,Rslt));
		}
		else
		{
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(GETADDR,Op1,Rslt));
		}
	}
...
bool semantic061()
{
	...
	if (i!=-1)
	{
		if (!SymbolTbl.LabelInfoTbl[i].m_bDef)
		{
			...
			Tmp.m_iLink=i;
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(LABEL,Tmp));
...
bool semantic063()
{
	...
	if (i!=-1)
	{
		...
		Tmp.m_iType=OpInfo::LABEL;
		Tmp.m_iLink=i;
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(JMP,Tmp));
...
bool semantic066()
{
	...
	for (int i=SymbolTbl.ProcInfoTbl.at(Tmp.m_iProcId).m_ParaTbl.size()-1;i>=0;i--)
	{
		...
		if (CType::TypeCompatible(Tmp.m_Paras.at(i),Op,17)!=-1)
		{
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
						.m_Codes.push_back(EmitIR(PARA,Tmp.m_Paras.at(i),Op));
		}
		else
		{
	...
	{
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(CALL,Op));
	}
	else
	{
		OpInfo Op=Operand.top();
		Operand.pop();
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(CALLPTR,Op));
	}
	...
	if (SymbolTbl.ProcInfoTbl.at(Tmp.m_iProcId).m_iReturnType>=0)
	{
		...
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(RETV,TmpOp,TmpOp,Rslt));
		Var TmpVar;
		TmpVar.m_iDim=0;
...
bool semantic068()
{
	...
	else
	{
		...
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(JNT,Tmp,TmpS.GetLabel(Statement::LabelIdx::FalseLabel)));
	}
...
bool semantic069()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(JMP,TmpS.GetLabel(Statement::LabelIdx::ExitLabel)));
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(LABEL,TmpS.GetLabel(Statement::LabelIdx::FalseLabel)));
...
bool semantic070()
{
	...
	{
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(LABEL,TmpS.GetLabel(Statement::LabelIdx::ExitLabel)));
	}
	else
	{
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
...
bool semantic071()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(LABEL,TmpS.GetLabel(Statement::LabelIdx::EntryLabel)));
...
bool semantic072()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(JMP,TmpS.GetLabel(Statement::LabelIdx::EntryLabel)));
...
bool semantic073()
{
	...
	else
	{
		...
		TmpS=CurrentStatement.top();
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(JNT,Tmp,TmpS.GetLabel(Statement::LabelIdx::ExitLabel)));
	}
...
bool semantic075()
{
	...
	else
	{
		...
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
					.m_Codes.push_back(EmitIR(JNT,Tmp,TmpS.GetLabel(Statement::LabelIdx::EntryLabel)));
...
bool semantic077()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
				.m_Codes.push_back(EmitIR(LABEL,CurrentStatement.top().GetLabel(Statement::LabelIdx::EntryLabel)));
...
bool semantic078()
{
	...
	OpInfo TmpRslt;
	TmpRslt.m_iType=OpInfo::VAR;
	TmpRslt.m_iLink=SymbolTbl.GetTmpVar(SymbolTbl.ProcStack.top(),(StoreType::CStoreType)SymbolTbl.TypeSysTbl.at(TmpResult).m_iRsltType);
	OpVarSemantic(Op1,Op2,TmpRslt,SymbolTbl.TypeSysTbl.at(TmpResult));

	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
				.m_Codes.push_back(EmitIR(JNT,TmpRslt,CurrentStatement.top().GetLabel(Statement::LabelIdx::ExitLabel)));
	return true;
}
...
bool semantic079()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
			.m_Codes.push_back(EmitIR(LABEL,CurrentStatement.top().GetLabel(Statement::LabelIdx::TrueLabel)));

	if (CurrentStatement.top().m_bIsDownto)
	{
		if (!GenIR(Op1,Op2,Op1,SUB))
		{
			return false;
		}
	}
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
			.m_Codes.push_back(EmitIR(JMP,CurrentStatement.top().GetLabel(Statement::LabelIdx::EntryLabel)));

	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
			.m_Codes.push_back(EmitIR(LABEL,CurrentStatement.top().GetLabel(Statement::LabelIdx::ExitLabel)));
...
bool semantic081()
{
	...
	while (i>=0)
	{
        tmpPState=getCurrentStatementElement(CurrentStatement,i);
		pState=&tmpPState;
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
				.m_Codes.push_back(EmitIR(JMP,pState->GetLabel(Statement::LabelIdx::ExitLabel)));
	return true;
}
...
bool semantic082()
{
	...
	{
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
				.m_Codes.push_back(EmitIR(JMP,pState->GetLabel(Statement::LabelIdx::TrueLabel)));
	}
	else
	{
		SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top())
				.m_Codes.push_back(EmitIR(JMP,pState->GetLabel(Statement::LabelIdx::EntryLabel)));
	}
...
bool semantic083()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(EmitIR(ASM,Op1,Op2,Op3));
...
bool semantic087()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(
				EmitIR(LABEL,CurrentStatement.top().m_Labels.back().m_Label));
...
bool semantic088()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(
				EmitIR(JMP,CurrentStatement.top().GetLabel(Statement::LabelIdx::ExitLabel)));
...
bool semantic089()
{
	...
	SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes.push_back(
				EmitIR(LABEL,CurrentStatement.top().GetLabel(Statement::LabelIdx::ExitLabel)));

	for (int i=CurrentStatement.top().m_Labels.size()-1;i>0;i--)
	{
		Op2.m_iLink=CurrentStatement.top().m_Labels.at(i).m_iConst;
		TmpIRList.push_back(
				EmitIR(OpType(JE_1+iOpSize/2),Op1,Op2,CurrentStatement.top().m_Labels.at(i).m_Label));
	}
	...
	while (!TmpVarItem.empty())
	{
		...
		if (CType::IsOrd(Op2))
		{
			...
			SymbolTbl.ProcInfoTbl.at(SymbolTbl.ProcStack.top()).m_Codes
					.push_back(EmitIR(SET_ADDITEM,Op1,Op2,Rslt));
			Operand.push(Rslt);
		}
		else
		{
...
bool OpConstFold(OpInfo Op1,OpInfo Op2,int iProcessId,int iReturnType,OpInfo &Rslt)
{
	...
	Rslt.m_iType=OpInfo::CONST;
	RsltConst.m_StoreType=(StoreType::CStoreType)iReturnType;
	...
	switch ((StoreType::CStoreType)iReturnType)
	{
...
WithField getWithStackElement(stack<WithField> WithStack, int position)
{
        WithField result = WithStack.top();
        WithStack.pop();
        if (WithStack.size() == position)
        {
//            stack.push(result);
            return result;
        }else {
            WithField element = getWithStackElement(WithStack, position);
            WithStack.push(element);
            return element;
        }
}

Statement getCurrentStatementElement(stack<Statement> CurrentStatement, int position)
{
        Statement result = CurrentStatement.top();
        CurrentStatement.pop();
        if (CurrentStatement.size() == position)
        {
//            stack.push(result);
            return result;
        }else {
            Statement element = getCurrentStatementElement(CurrentStatement, position);
            CurrentStatement.push(element);
            return element;
        }
}
